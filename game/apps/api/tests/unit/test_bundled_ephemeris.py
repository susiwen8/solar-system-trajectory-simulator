...
def test_bundled_ephemeris_supports_long_range_keplerian_lookup() -> None:
    data_path = Path(__file__).resolve().parents[2] / "data/ephemeris/major_bodies.json"
    ephemeris = BundledEphemeris(data_path)

    state = ephemeris.get_body_state("jupiter", "2026-09-01T00:00:00Z")

    assert state.body_id == "jupiter"
    assert len(state.position_km) == 3
    assert len(state.velocity_km_per_s) == 3
    assert state.mu_km3_per_s2 > 100_000_000
