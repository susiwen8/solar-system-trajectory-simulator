...
it("shows the mission metrics panel after propagation results load", async () => {
  ...
  expect(await screen.findByTestId("scene-focus-card")).toHaveTextContent("火星");
...
it("renders gravity-assist candidates and switches the active plan", async () => {
  ...
  expect(await screen.findByTestId("scene-focus-card")).toHaveTextContent("木星");
  expect(await screen.findByTestId("scene-focus-card")).toHaveTextContent("飞越增强视角");
});
